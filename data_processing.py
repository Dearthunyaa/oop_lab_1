...
class DB:
    ...
    def insert(self, my_value):
        self.mytable[my_value.table_name] = my_value.table

    def search(self, tablename):
        return Table(tablename, self.mytable[tablename])
...
class Table:
    ...
    def filter(self, condition):
        ...
        return Table(table_name=self.table_name + "_filtered", table=filtered_list)
...
print(my_table2.filter(lambda x: x['coastline'] == 'yes')
...
my_table3_filtered = my_table3.filter(lambda x: x['EU'] == 'no') \
...
my_table3_filtered = my_table3.filter(lambda x: x['EU'] == 'yes') \
